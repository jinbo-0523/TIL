...
ネットワーク構成の設計
ネットワークの構築をおこな場面は、ネットワークを追加するケースと、ゼロから作るケースが考えられる。きちんと設計することで現状のニーズだけでなく将来のニーズにも対応できるネットワークを構築する事ができる。

・設計の手順
ネットワークの設計には考慮するべき点が多くある。
①利用人数、②利用曜日と時間帯、③接続端末数、④接続方法(有線・無線)、⑤ユーザー種別(従業員・来客)、⑥利用するアプリケーション、⑦利用するWAN回線、⑧セキュリティ上の注意事項などを調べる。
調査が終わったら①LANの構成、②WANの構成、③セキュリティ、④監視の順でせいっ系を始める。



サブネット構成とIPアドレス割り当て
組織のネットワークは特殊な場合を除き、小さなサブネットに分割してそれぞれを接続する構成にしている。調査しておいた部署ごとの端末状況をもとにどのように分割するかを検討する。通常は同じ部署を１つのサブネットとするにが一般的になる。
【おさらい：サブネットマスク　IPアドレスは32ビット(32桁)のうちの前からO行目までのネットワーク部のこと】

・IPアドレス割り当て
サブネット分割方針が決まったら各サブネットにネットワーク割り当てる。プライベートアドレスを割り当てる。その際には部署ごとの端末数に対してIPアドレスに十分な余裕があるよう、サブネットマスクを決める。
プレフィックス長は各サブネットで同じ値にしておく方が、運用では楽なことが多い
【おさらい：プレフィックス長　ネットワーク部が前から何桁目までかをIPアドレスの右に/24みたいな感じで書いてあるもの】

・ルーティング
通信させないサブネット間なども出てくる。最終的にはこの内容を各ルーターにルーティングテーブル、あるいはフィルタに反映されることになる。


ハードウェアとソフトウェアの選択
ネットワーク機器の評価ポイントには①安定性②相互接続性③性能④機能⑤サポート⑥価格。
設置台数が多いスイッチやハブについて消費電力も考慮する必要がある。


・サーバーの選択
サーバーの選択は『クラウドにするかオンプレミス(組織内にサーバーを持つこと)にするか』から検討を始める。
クラウドについてはセキュリティ、社内ネットワークからの安全なアクセス方法、反応速度、従量課金の場合は利用料の程度等を検討する。
オンプレミスでは適切な空調が整った設置場所の用意、停電や災害の対策、バックアップのやり方、等を検討する。
サーバーの使用は、CPUの種類、メモリの容量、ハードディスクの容量、RAIDの方式、ネットワークインターフェースの速度などが使用目的に合致している物を選択する。クラウドのサーバーは、契約後でも使用の一部を自由に変更できるものが多いことが特徴。
サーバーに使用するOSの種類は自由に選べるならLinuxの管理スキルがあればLinuxを選び、そうでなければwindowsを使うことがほとんど。


インターネットとの接続
いくつか方法があるが(A)アクセス回線サービスと(B)ISPサービスのふたつを組み合わせて使用するのは基本形。
A・・・NTTやKDDIなどの通信事業者が提供
B・・・プロバイダーと言われるインターネット接続事業者が提供
・・・
①AとB、別々に申し込む
②AのセットでBを申し込む
③BのセットでAを申し込む
④(C)光コラボレーション事業者に申し込む
C・・・NTTなどから回線サービスを仕入れ自社サービスとして販売している会社のこと

・アクセス回線で確認しておきたいこと
アクセス回線とは最寄りの電話局からオフィスや家庭まで繋がる光回線の事。料金や最低契約期間の他に①設置場所がサービス提供地域か、②開通が間に合うかどうか、③指定のルーターを使う場合はその性能や機能が要件に適しているか、④設置場所で使用できる屋内配線の種類などを確認。確認自体は申し込む会社に質問すれば教えてもらえる。

・ISPの選び方
インターネットを利用するチキの実効速度はISPサービスによっても変わるが、その速度を申し込み前に計る方法はないためスピードテストサイト等を参考に決める。
...
サーバーの公開
インターネットに公開する方法としては、①組織の建物内にサーバーを置く②データセンターにサーバーを置く③クラウドサーバーを利用、があげられる。
組織の建物内にサーバーを置く場合、安定した空調と電源を確保できるスペースにサーバーを設置すると共にファイアウォールにDMZを設け、そこに接続する。
【おさらい：DMZ　インターネットと公開サーバーの間に挟む中間保護地点。ファイアウォールやフィルタリングをの機能を持たせる】

・固定IPアドレス
公開するサーバーは外部からアクセスできる必要があるため、固定化されたIPアドレスを割り当てる。

・ドメイン名の取得とDNSの設定
公開するサーバーには多くの場合ドメイン名を割り当てる。ドメイン名はレジストラやリセラで利用で取得したドメイン名は実際に使うにはドメイン名のためにDNSサーバーを最低でも2台用意して、そのIPアドレスを登録申請する必要がある。


Windowsのワークグループとドメイン
・ワークグループの概要
Windowsのネットワークでは①ワークグループと②ドメインというという２種類のユーザー管理のスタイルがある。
①ワークグループとは、同じワークグループ名を設定しているコンピューター同士で一覧や共有フォルダの一覧等を見ることができるもので、ユーザー名とパスワードはそれぞれのコンピューターで管理している。パスワードを変更しても他のコンピューターには反映されない。そのため管理が煩雑になるワークグループは小規模なネットワークでよく使われる。


・ドメインの概要
②ドメインではユーザー名とパスワードをドメインコントローラーと呼ばれるサーバーで集中管理する。各コンピューターはドメインに参加することによりアクセスが可能になり登録されているユーザー名とパスワードでユーザー認証を行えようになる。
ドメインでは集中管理されているため、パスワードが変更すれば全ての認証に反映されるそのためドメインは大規模なネットワークに用いられる。



ディレクトリサービス
コンピューターやネットワーク機器の所在、固有情報、設定などの対応を集積しておき、それを提供するサービスをディレクトリサービスという。小規模なネットワークは情報の管理が簡単なため、比較的規模が大きいネットワークで利用される。

・ディレクトリサービスで扱う情報
扱える情報としては①ユーザーID、②パスワード、③メールアドレス、④共有フォルダ、⑤、⑥サーバー情報あどがあり、ログイン処理を行う際に、ユーザー情報をもつデータベースとして参照される事がある。この場合、サービスが停止するとログインできないといった重大故障に結びつく。
そこでディレクトリサーバーを複数台設置したり、分散して設置するなどして対策をする。

・ディレクトリサービスの種類
ディレクトリサービスにアクセスするプロトコルとしてはLDAPが多く用いられる。主なディレクトリサービスにはLinux・Windows・MacOSなどのオープンソースの『Open LDAP』、Windows Server に搭載された『Active Directory』、 Mac OS XServerに搭載された『Open Directory』などがある。
 


LAN配線の敷設と加工
オフィスのLAN配線は、自分でやる方法と専門業者に任せる方法がある。
・専門業者に任せる方法
専門業者に依頼する場合、打ち合わせ・現地調査・見積り・契約/発注・施工・保守という流れになる。

・自分で配線工事をするときのポイント
自分でやる場合、資材選びから始める。LANケーブルには市販の完成品を使う方法と
必要な長さに切りコネクタを取り付ける方法がある。
圧着工具を使って両端にLANコネクタを圧着しケーブルテスターで正しく加工できたかチェックし、ケーブルとコネクタは使用するイーサネットの規格に合わせて選択する。
同じイーサネットの規格のケーブルであっても、細いものや平たくなっている物などがある。
単線とより線の２種類があり、単線の方が伝送効率が良いが、力に弱いため取り扱いには注意が必要。また、ケーブルをなるべく同じルートを通してモール使ってまとめる。



安定した電源の確保
安定稼働には安定した電力の供給が欠かせない。以下が電源供給のトラブルになる。
①停電・・・電力供給が止まること。ルーターやスイッチなどで動作停止の問題はないが、コンピューターや共有ストレージはデータの消失・破損などが起こる。

②瞬断・・・短い時間、商用電源が途切れる現象のことで、電力系統の切り替え工事などで発生する。停電と似たトラブルが発生する。

③電圧降下・・・大電流が急激に流れる機器があると機器起動時に一瞬電圧がだがることもある。電圧降下は機器の誤作動を引き起こす要因になる

④ノイズ・・・外部からのエネルギーが電力線に混入し発生する。これも誤作動の原因となる。

⑤スパイクやサージ・・・短時間に異常な高電圧がかかる現象で、短いものをスパイク・長めのものをサージという。原因は雷や大電力を消費する機器の急停止が考えられる。スパイクやサージは電子回路の破壊やデータの消滅など、機器に深刻な被害を与える恐れがある。

・対策
ノイズフィルタやサージプロテクタを備えたテーブルタップの使用や、UPS(無停電電源装置)の導入が考えられる。
...
ネットワークの冗長化
LANケーブルの断線やトラブル等に備えて、単純にLANが繋がるルートを二重にすると、ブロードキャストフレームが無限に回り続ける「ブロードキャストストーム」が、発生しネットワークがダウンしてしまう。
ブロードキャストを受けとるとスイッチはそれを受信ポート以外のすべてのポートに送り出す。この動作を各スイッチが行う際、ループする部分があるとブロードキャストフレームが、永遠に回り続ける異常事態が発生する。
ネットワークの帯域を消費し尽くし、通信を困難になるほか、大量のブロードキャストを受信したコンピューターは負荷が上昇する。

・ブロードキャストストームを回避するには
とはいえ冗長化自体は有効なアプローチのひとつでもある。複数のルートを作ってもブロードキャストストームが発生はしないスパニングツリープロトコル(STP)が考え出された。これを使えばスイッチがループの生じるポートを無効にしてブロードキャストストームを防ぎ、ルートが故障した際、無効にしたポートを有効にして通信を再開する。近年ではすいっちのスタッキングなどにも、使われている。
複数のスイッチを積み上げ(スタック)し、1台のスイッチのように見せ子スイッチを繋ぎ使用する。この構成ではループが生じないためスパニングツリープロトコルが、不要でかつネットワークの冗長化が実現できる。


ネットワークの監視
正常に動作しているかどうかを見張ることを『ネットワークの監視』と呼ぶ。

・死活監視と状態監視
外部から監視する時に用いられるのが①死活監視と②状態監視の２種類がある。
なおそれ以外の専用監視エージェントなども使いネットワーク全体監視するソフトウェアもある（ZabbixやNagiosなど）

①死活監視
対象機器にPING（正確にはICMP ECHO要求）を送り、受け取った機器が送り返す応答を確認、機器が動作しているとみなす監視方法になる。
PINGがなかった場合、対象機器か経路ネットワークに問題があり、これを確認する方法は同じ経路で辿り着ける別の機器も同時に監視する方法がある。
片方の機器からだけ応答が得られたらネットワークは正常で、機器に問題があると言った推測が可能になる。

②状態監視
対象機器が内部に持っている統計情報を読み出して、機器の正常性を確認したり以上を見つけ出す方法である。
死活監視が単に『動いている/動いていない』だけを見るのに対して、状態監視は、『機器の負荷率が一定値を超えた』『ネットワークの混雑どが基準をを突破した』など、監視の仕組みは複雑になるるが、機器の動作に着目して監視することができる。

・SNMPと監視システム
統計情報を読み出す手段にはSNMPがよく使われる。SNMPでは①定期的な外部から情報を読み出す『ポーリング』、②機器が自律的に情報を送り出す『トラップ』の２種類があり、目的に応じて使い分けを行う。
...
トラブルシューティング
ネットワークの動作には多くの要素が関与していて『サーバーにアクセスできない。ネットワークがおかしい。』という申告１つとってもその原因にはsまざまな理由が考えられる、そこでトラブル発生時には『切り分け』を行う。『切り分け』とは、
『不具合が発生した時、正常に動いている部分と不具合のある部分の教会を明確にし、少しずつ少しずつ移動させながら、不具合のある箇所を絞り込んで特定すること』
をいう。

・PINGコマンドを使った到達性に関する切り分け
pingコマンドはコンピューターからネットワーク機器やサーバーに対してICMP ECHOパケットを送りそれに対して応答を返すかどうかを調べるコマンドになる。
PINGの応答があるということは、LANケーブル、スイッチ、ルーター、ルーティングなどが正常であることを意味する。

・digやnslookupコマンドを使った名前解決の可否の切り分け
ドメイン名で指定された相手と通信をするときは、まずドメイン名をIPアドレスに変換する名前解決を行い、その後相手との通信を開始する。この一連の動作がどこまで正常に進んでいるか確かめることも切り分けの一つになる。
例えば「通信ができない」という症状かが現れたらまず、digコマンドやnslookupコマンドで名前解決の正常性を確認し、それが正常なら、次の相手までの到達性を確認する。
